
/*

    all the nfd related operations will be checked by this file

 */

var path_ndn = '/Users/saurabdulal/iCloud Drive (Archive)/Documents/COURSE_UofM/FALL_18/Operating_System/MajorProject/TEST/ndn-js';

var nodeConsole = require('console');
var con = new nodeConsole.Console(process.stdout, process.stderr);


var Face = require(path_ndn).Face;
var Name = require(path_ndn).Name;
var Data = require(path_ndn).Data;
var Blob = require(path_ndn).Blob;
var UnixTransport = require(path_ndn).UnixTransport;
var SafeBag = require(path_ndn).SafeBag;
var KeyChain = require(path_ndn).KeyChain;
var readline = require('readline');

var face = new Face(new UnixTransport() );


var DEFAULT_RSA_PUBLIC_KEY_DER = new Buffer([
    
]);

// Use an unencrypted PKCS #8 PrivateKeyInfo.
var DEFAULT_RSA_PRIVATE_KEY_DER = new Buffer([
    
]);

var Echo = function Echo(keyChain, face) {

    this.keyChain = keyChain;
    this.face = face;
};


Echo.prototype.onInterest = function(prefix, interest, face, interestFilterId, filter)
{

    // construct and sign the data packet
    /*
       when we receive interest under our prefix, we do following
       1. check if it contains any sequence number of not
       2. if it contains sequence number, send birb corresponding to that sequence number
       3. else send the latest sequence number as the data
    */
    var interest_name = interest.getName().toUri();
    var data = new Data(interest.getName());
    var content;
    try {
        var split_list = interest_name.split("seq");
        var seq_number = parseInt(split_list[1].split("/")[1]);
        // process for specific sequence number
        content = get_row(seq_number)[0].birb_text;
        // con.log(content)
    }
    catch(err) {
        // process for no sequence number present
        content = get_birb(L_user_detail_table_name)[0].last_seqnumber
        // con.log(content)

    }

    data.setContent(content);
    this.keyChain.sign(data);

    try {
        con.log("Sent content " + content);
        face.putData(data);
    } catch (e) {
        con.log(e.toString());
    }
    //this.face.close();  // close face if you want to quit the script
};


Echo.prototype.onRegisterFailed = function(prefix)
{
    con.log("Register failed for prefix " + prefix.toUri());
    //this.face.close();  // close face if you want to quit the script
};


function main(username)
{
    // connection to local NFD with Unix Socket
    // var face = new Face(new Transport("UnixTransport"));
    // var face = new Face(new UnixTransport() ); //, {host: "localhost", port: 9696});
    con.log(face);

    /*
       For now, when setting face.setCommandSigningInfo, use a key chain with
       a default private key instead of the system default key chain. This
       is OK for now because NFD is configured to skip verification, so it
       ignores the system default key chain.
    */
    let user_prefix = "/birb/"+username;

    var keyChain = new KeyChain("pib-memory:", "tpm-memory:");
    keyChain.importSafeBag(new SafeBag
    (new Name("/testname/KEY/123"),
        new Blob(DEFAULT_RSA_PRIVATE_KEY_DER, false),
        new Blob(DEFAULT_RSA_PUBLIC_KEY_DER, false)));

    face.setCommandSigningInfo(keyChain, keyChain.getDefaultCertificateName());

    var echo = new Echo(keyChain, face);
    var prefix = new Name(user_prefix);
    con.log("Register prefix " + prefix.toUri());
    face.registerPrefix(prefix, echo.onInterest.bind(echo), echo.onRegisterFailed.bind(echo));
}

function start_listening_on_prefix(username){
    main(username);

}

function check_nfd_status(){
    // here we check if nfd is started or not,
    // if not started, we will start it
}

function create_face(){
    // create face from local nfd to remote nfd
    // before check if face exists or not
}



var onTimeout = function (interest) {
    console.log("Time out for interest " + interest.getName().toUri());
    // face.close();  // uncomment if you want to close the face
};

function request_data_from_producer(tableName) {


    var tn = "R_"+i_f[i]+"_details";

    if (get_birb(tn).length == 0);
    {
        console.log("no data in this user name");
        //get all the sequence number published
        var interest_prefix = "/birb/"+i_f[i]+"/";
        console.log(interest_prefix);
        request_data_from_producer(interest_prefix, onData)

    }

    var read_lines = readline.createInterface({
        input: process.stdin,
        output: process.stdout
    });

    var name = new Name(prefix);
    console.log("Express name " + name.toUri());
    face.expressInterest(name, onData, onTimeout);

}


module.exports = {
    check_nfd_status,
    create_face,
};
